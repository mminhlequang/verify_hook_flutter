...
class TRXScreen extends StatefulWidget {
  ...
  const TRXScreen({super.key, this.platformId, this.identifierCode});

  @override
  State<TRXScreen> createState() => _TRXScreenState();
}

class _TRXScreenState extends State<TRXScreen> {
  final TextEditingController transactionIdController = TextEditingController();
  late final TextEditingController messageController =
      TextEditingController(text: widget.identifierCode);
  ...
  Future<void> verifyTransactionId(String transactionId) async {
    try {
      ...
      final url =
          'https://apilist.tronscan.org/api/transaction-info?hash=$transactionId';
      ...
      if (response.statusCode == 200) {
        ...
        if (data['confirmed'] == true) {
          ...
          if (data['contractRet'] == 'SUCCESS') {
            final amount =
                int.parse(data['trc20TransferInfo'][0]['amount_str']) / 1000000;
